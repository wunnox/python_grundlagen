...
pyodbc.pooling = True
connectionF = pyodbc.connect("DRIVER={/Library/ODBC/FileMaker ODBC.bundle/Contents/MacOS/fmodbc.so};encoding=utf-8;SERVER=localhost;DATABASE=Schulung;UID=python;PWD=python12")
connectionF.setencoding(encoding='utf-8')
connectionF.setdecoding(1, encoding='ISO8859-1', ctype=1)
...
cursor.execute(
    "select Termin from sort where Termin!='' and Termin!=' ' and Termin!='None' order by Termin")
for row in cursor:
    migdat.append(row[0])
...
# Excel erstellen
ExcelName = "/Users/peter/Dropbox/Python/Dokumente/DBM Migrationen Planung KW" + kw + ".xlsx"
...
dbaname = "None"
cursor.execute(
    "select * from odbc where Status != 'Keine Migration' order by Sort,SID")
for row in cursor:
    col = colsize(coln)
    ce = col + str(z)
    ws1[ce].value = row[0]

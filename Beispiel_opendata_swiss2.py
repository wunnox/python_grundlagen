    ...
    try:
        ...
        for evse_data in data['EVSEData']:
            for record in evse_data['EVSEDataRecord']:
                ...
                plz = record.get('Address', {}).get('PostalCode')
                if 'bern' in city and '3' == plz[0]:
        ...
        if bern_stations:
            ...
            for i, station in enumerate(bern_stations, 1):
                ...
                print(
                    f"Name: {station.get('ChargingStationNames', [{}])[0].get('value', 'N/A') if station.get('ChargingStationNames') else 'N/A'}")
                print(
                    f"Stadt: {station.get('Address', {}).get('City', 'N/A')}")
                print(
                    f"PLZ: {station.get('Address', {}).get('PostalCode', 'N/A')}")
                print(
                    f"Straße: {station.get('Address', {}).get('Street', 'N/A')}")
                print(
                    f"Koordinaten: {station.get('GeoCoordinates', {}).get('Google', 'N/A')}")
                print(f"24h geöffnet: {station.get('IsOpen24Hours', 'N/A')}")
                print(
                    f"Stecker-Typen: {', '.join(station.get('Plugs', ['N/A']))}")

                # Ladeleistung anzeigen
                facilities = station.get('ChargingFacilities', [])
                if facilities:
                    powers = [
                        f"{f.get('power', 'N/A')}kW ({f.get('powertype', 'N/A')})" for f in facilities]
        ...
        if bern_stations:
            ...
            print(
                f"Eine formatierte Version wurde in '{output_file_pretty}' gespeichert.")

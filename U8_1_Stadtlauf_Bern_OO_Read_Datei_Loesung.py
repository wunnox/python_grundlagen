...
w=open('Wegdaten_OO.txt','r')
alleschritte=w.readlines()
w.close()
...
##############################################
       ...
       for schritte in alleschritte:
          gx,gy,gl,gr=schritte.rstrip().split(":")
          time.sleep(slower)  #Laufgeschwindigkeit reduzieren
          if gl=='True':
             x,y,left,right=marsi.go_walk_left(int(gx),int(gy))
          elif gr=='True':
             x,y,left,right=marsi.go_walk_right(int(gx),int(gy))
